...
for (var i = 0; i < x.length; i++) {
    ...
    var weight = ~~line[1];
     ...
     if (parent == 'mdbtyw') {
         weight = weight - 5;
        console.log("Adjusted weight to ", weight);
    }

    console.log(line);

    if (tree[parent] == undefined) {
        tree[parent] = { weight: weight, children: [], parent: "" };
    } else {
...
function getChildrensWeight(key, level) {
    var weight = tree[key].weight;
    console.log(key, weight);
    for (var i = 0; i < tree[key].children.length; i++) {
        weight = weight + getChildrensWeight(tree[key].children[i], level + 1);
    }
    console.log(key, weight, level);
    if (levels[level] == undefined) {
        levels[level] = [];
    }
    tree[key].totalWeight = weight;
    tree[key].level = level;
    levels[level].push({ key: key, weight: weight });
    return weight;
}
...
function checkBalance(key) {
    ...
    var weights = [];
    for (var i = 0; i < tree[key].children.length; i++) {
        var child = tree[key].children[i];
        var newWeight = tree[child].totalWeight;
        if (weight == 0) {
            weight = newWeight;
        }
        if (newWeight != weight) {
        ...
        checkBalance(tree[key].children[i]);
    }
    if (mismatch) {
        console.log(key, weight, mismatch);
        console.log(keys);
        console.log(weights);
    }
}

getChildrensWeight(top, 1);
console.log("-----BALANCE-----");
checkBalance(top, 1);